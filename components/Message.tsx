...
function CodeBlock({ children, inline, className }: any) {
  ...
  return (
    <div className="relative group my-2">
      ...
      <code className="bg-black/30 px-2 sm:px-3 py-2 pr-12 rounded block overflow-x-auto text-xs">
        {children}
      </code>
    </div>
  );
...
export default function Message({
  ...
  if (role === "user") {
    ...
    return (
      <div className="flex justify-end flex-col items-end">
        <div
          className="max-w-full sm:max-w-2xl px-3 sm:px-4 lg:px-5 py-3 sm:py-4 rounded-xl sm:rounded-2xl border border-white/10 bg-emerald-500/20 cursor-pointer"
          tabIndex={0}
        ...
        >
          {isEditing ? (
            ...
            <div className="text-xs sm:text-sm leading-relaxed prose prose-invert max-w-none break-words overflow-x-auto">
              <ReactMarkdown
        ...
        {!isEditing && showEdit && (
          <button
            onClick={handleEdit}
            className="mt-2 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-xs text-emerald-900 border border-emerald-200"
            title="Edit message"
          >
  ...
  // Assistant message: only code blocks in box, text is plain
  return (
        ...
        <div className="text-xs sm:text-sm leading-relaxed prose prose-invert max-w-none break-words overflow-x-auto">
          <ReactMarkdown
            ...
            components={{
              ...
              ol: ({ node, ...props }) => <ol className="list-decimal list-inside mb-2 space-y-1" {...props} />,
              li: ({ node, ...props }) => <li className="mb-1" {...props} />,
              code: (props) => <CodeBlock {...props} />, // CodeBlock already renders box for code
              blockquote: ({ node, ...props }) => (
                <blockquote className="border-l-4 border-indigo-500 pl-2 sm:pl-3 italic my-2 opacity-75" {...props} />
              ),
              a: ({ node, ...props }) => <a className="text-indigo-400 underline hover:text-indigo-300 break-all" {...props} />,
